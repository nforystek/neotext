...
Public Sub soSweetPaserPresets()
    ...
    ParserMessages(ParserCodes.ErrMsg_00) = "Expecting HOOPS statement as script start."
    ...
    ParserMessages(ParserCodes.ErrMsg_20) = "External errors prevented the scripts execution."
...
Public Function lArrayOfANSI(ByRef ansi As MemInfo) As Byte() 'local
    'converts the ansi to array making it able native array use
    Dim out() As Byte
    ReDim out(0 To lLengthANSI(ansi) - 1) As Byte
