...
Public Function InputLoop() As Long
    Dim cnt As Long
    For cnt = 0 To 255
        If (Not (VirtualKeys(cnt).VKState = GetKeyState(cnt))) Then
            VirtualKeys(cnt).VKState = GetKeyState(cnt)
            ...
            Else
                VirtualKeys(cnt).VKPressed = False
            End If
        ...
        Else
            If (VirtualKeys(cnt).VKLatency <> 0) Then
                If ((Timer - -VirtualKeys(cnt).VKLatency) > 0.1) And (VirtualKeys(cnt).VKLatency < 0) Then
                    VirtualKeys(cnt).VKPressed = True
                    VirtualKeys(cnt).VKLatency = VirtualKeys(cnt).VKLatency + -0.1
                ElseIf ((Timer - VirtualKeys(cnt).VKLatency) > 0.6) And (VirtualKeys(cnt).VKLatency > 0) Then
                    VirtualKeys(cnt).VKPressed = True
                    VirtualKeys(cnt).VKLatency = VirtualKeys(cnt).VKLatency - 0.6
                Else
                    VirtualKeys(cnt).VKPressed = False
                End If
            Else
...
End Function
'Public Property Get Pressed(ByVal vkCode As Long) As Boolean
'    Pressed = VirtualKeys(vkCode).VKPressed
'End Property
'Public Property Let Pressed(ByVal vkCode As Long, ByVal RHS As Boolean)
'    VirtualKeys(vkCode).VKPressed = RHS
'End Property
