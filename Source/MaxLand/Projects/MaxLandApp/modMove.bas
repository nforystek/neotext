...
Public Function AddMotion(ByRef Motions As NTNodes10.Collection, ByRef Action As Long, ByVal Key As String, ByRef Data As Point, Optional ByRef Emphasis As Single = 0, Optional ByVal Friction As Single = 0, Optional ByVal Reactive As Single = -1, Optional ByVal Recount As Single = -1, Optional Script As String) As String
    ...
    With act
        ...
        .latency = Timer
        .Recount = Recount
        If Not IsMissing(Script) Then
            .Script = Script
        Else
            'Set .Script = New NTStreams10.Stream
        End If
        AddMotion = .Key
    End With
...
Private Sub ApplyMotion(ByRef obj As Element, ByVal Action As Actions)
    ...
    If Not Camera.Element Is Nothing Then
        ...
        If ((Not (Perspective = Spectator)) And (obj.CollideObject = Camera.Element.CollideObject)) Or (Not (obj.CollideObject = Camera.Element.CollideObject)) Then
...
Public Sub ResetMotion()
    Dim A As Long
    Dim o As Long
   ' If Not Camera.Element Is Nothing Then Set Camera.Element.Direct = MakePoint(0, 0, 0)
...
On Error GoTo ObjectError
    ...
    'If Not Camera.Element Is Nothing Then RenderMotion2 Camera.Element
...
On Error GoTo ObjectError
    ...
    If Not Camera.Element Is Nothing Then
        ...
        If ((Perspective = Spectator) Or DebugMode) Then
            ...
            Camera.Element.Origin.X = Camera.Element.Origin.X + Camera.Element.Direct.X
            Camera.Element.Origin.Y = Camera.Element.Origin.Y + Camera.Element.Direct.Y
            Camera.Element.Origin.Z = Camera.Element.Origin.Z + Camera.Element.Direct.Z
                    
        Else
        
            'InputMove2 Camera.Element
            ...
            If (e1.Effect = Collides.Normal) And (obj.CollideIndex > -1) Then
                If (Not e1.CollideObject = obj.CollideObject) Then
                    ...
                    'if found to be with the colliding object
                        ...
                        e1.Direct.X = obj.Direct.X
                        e1.Direct.Y = obj.Direct.Y
                        e1.Direct.Z = obj.Direct.Z 'setting this seems to "magnetically" couple the directive
...
'    Rotator = Rotator + IIf(Camera.Rotate.Y > 0, testNudgeAdjust, -testNudgeAdjust)
...
'                Set p = VectorRotateY(VectorRotateX(MakePoint(0, 0, 1), Camera.Rotate.X), Camera.Rotate.Y)
'            Else
'                sngCamera(0, 0) = 0
'                sngCamera(0, 1) = 0
'                sngCamera(0, 2) = 0
...
Public Sub RenderPortals()
    ...
    Do While cnt <= Portals.Count
        ...
        If Portals(cnt).Enabled Then
            ...
            'If Not Camera.Element Is Nothing Then RenderPortals2 Portals(cnt), Camera.Element
            ...
            If InStr(LCase(t1.OnInRange.AppliesTo) & ",", LCase(e1.Key) & ",") > 0 Or t1.OnInRange.AppliesTo = "" Then
                ...
                If (((t1.OnInRange.EventFlags And 1) <> 1) And (t1.OnInRange.Behavior = Press)) Or (t1.OnInRange.Behavior = Rapid) Then
                    ...
                    If (t1.OnInRange.Behavior <> Locks) Or ((t1.OnInRange.Behavior = Locks) And ((t1.OnInRange.EventFlags And 2) = 2)) Then
                        ...
                        Debug.Print "OnInRange " & t1.Key & " " & e1.Key & " Running " & t1.OnInRange.RunScript
                    End If
                End If
...
End Sub
Private Sub ExecuteScript(ByRef e1 As Element, ByRef EventText As String)
    Dim trig As String
    Dim line As String
    Dim id As String
    ...
    If Not Camera.Element Is Nothing Then
        If Cameras.Count > 0 Then
            ...
            For cnt = IIf(toggle, 1, Cameras.Count) To IIf(toggle, Cameras.Count, 1) Step IIf(toggle, 1, -1)
                Dist = DistanceEx(Camera.Element.Origin, Cameras(cnt).Origin)
                ...
                Do
                    ...
                    If (cnt > 0) Then
                        With Cameras(cnt)
                            ...
                            verts(0) = ToVector(Camera.Element.Origin)
                            verts(1) = VectorAdd(ToVector(Camera.Element.Origin), MakeVector(0, -0.01, 0))
                            ...
                            If (ClassifyPoint(V1, V1, V1, ToVector(Camera.Element.Origin)) = 1) Then touched = True
        
        
                            If Not touched Then
                                
                                
                                V1 = VectorSubtract(MakeVector(.Origin.X + Sin(D720 - .Rotate.Y), _
                                                                                .Origin.Y - Tan(D720 - .Rotate.X), _
                                                                                .Origin.Z + Cos(D720 - .Rotate.Y)), _
                                                                                ToVector(.Origin))
                                                                                
                                V2 = VectorSubtract(MakeVector(Camera.Element.Origin.X - Sin(D720 - .Rotate.Y), _
                                                                Camera.Element.Origin.Y + Tan(D720 - .Rotate.X), _
                                                                Camera.Element.Origin.Z - Cos(D720 - .Rotate.Y)), _
                                                                ToVector(.Origin))
                                ...
                                If ((V2.X > 0 And V1.X > 0) Or (V2.X < 0 And V1.X < 0)) And _
                                    ...
                                    If past <> 0 Then
                                        If DistanceEx(.Origin, Camera.Element.Origin) > Dist Then
                                            cnt = past
                                            Dist = DistanceEx(.Origin, Camera.Element.Origin)
                                        End If
                                    End If
