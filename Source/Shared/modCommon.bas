...
Public Function EnVarEval(ByVal URI As String) As String
    Do While URI <> "" And InStr(URI, "%") > 0
        EnVarEval = EnVarEval & RemoveNextArg(URI, "%")
        EnVarEval = EnVarEval & Environ(RemoveNextArg(URI, "%"))
    Loop
...
Public Function InvertNum(ByVal Value As Single, Optional ByVal Whole As Single = 100, Optional ByVal Unit As Single = 1)
...
Public Function FormatFileSize(ByVal fSize As String, Optional ByVal Abbrev As Boolean = False) As String
    ...
    FormatFileSize = VBA.Space(16 - Len(NewSize)) & NewSize & IIf(Abbrev, " B", " Bytes")
