Attribute VB_Name = "modCertificate"
Option Explicit
...
Private SnapShot As NTNodes10.Collection

Public Function LoadCertificateWindow(ByRef cert() As Byte) As Boolean

    Set SnapShot = New NTNodes10.Collection
    Dim idx As Long
    
    ItterateDialogs

    Dim tmpList As NTNodes10.Collection
    Set tmpList = SnapShot
    Set SnapShot = New NTNodes10.Collection
    
    Dim tmpFile As String
    tmpFile = GetTemporaryFile
    WriteFile tmpFile, ToString(cert)


    ItterateDialogs2 RunProcess("rundll32.exe", "cryptext.dll,CryptExtOpenCER " & tmpFile)

    
    If tmpList.count > 0 Then
        
        For idx = 1 To SnapShot.count
            If tmpList.Exists(SnapShot.Key(idx)) Then
                
                SnapShot.Remove tmpList.Key(idx)
    ...
    If SnapShot.count > 0 Then
        Dim nfrm As New frmThread

        LoadCertificateWindow = nfrm.CheckCertificateOK(SnapShot.Key(1))
        
        For idx = 1 To SnapShot.count
            Debug.Print SnapShot.Key(idx)
...
Private Function ItterateDialogs2(ByVal pId As Long) As String
    If (hWndIDE = 0) Then
...
Private Function SubCheckHwnds2(ByVal hwnd As Long, ByVal lParam As Long) As Boolean
    ...
    If lSize > 0 Then
        txt = Left$(txt, lSize)
    End If
    If InStr(txt, "Certificate") > 0 Then
...
Private Function SubCheckHwnds(ByVal hwnd As Long) As Boolean
    ...
    If (InStr(txt, "Certificate") > 0) Then
        ...
        If hWndUp <> 0 Then DoPrompt hWndUp
    End If
...
Private Sub DoPrompt(ByVal hwnd As Long)

        Debug.Print GetText(hwnd)
    If Not SnapShot Is Nothing Then

            If Not SnapShot.Exists("H" & hwnd) Then
                SnapShot.Add hwnd, "H" & hwnd
            End If
