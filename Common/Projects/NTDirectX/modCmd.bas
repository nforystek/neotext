...
Public Sub InputScene(ByRef UserControl As Macroscopic)
    ...
    If ((GetActiveWindow = UserControl.Parent.hwnd) Or (GetActiveWindow = frmMain.hwnd)) Then
        ...
        If ((TrapMouse Or FullScreen) And (Bindings.Controller = Trapping)) Or _
            ...
            If (Not (Screen.MousePointer = 99)) Then
                Screen.MousePointer = 99
                Set Screen.MouseIcon = PictureFromByteStream(LoadResData(2, "CUSTOM"))
            End If
        Else
            If (Not (Screen.MousePointer = 0)) Then Screen.MousePointer = 0
        End If
        ...
        If ((TrapMouse And (Not ConsoleVisible)) And (Bindings.Controller = Trapping)) Or _
            ...
            If (Bindings.Controller = Trapping) Then
                SetCursorPos (UserControl.Parent.Left / Screen.TwipsPerPixelX) + UserControl.Left + (UserControl.Width / 2), (UserControl.Parent.Top / Screen.TwipsPerPixelY) + UserControl.Top + (UserControl.Height / 2)
            End If
...
Public Sub CreateCmds()
    ...
    measure = (frmMain.Width / Screen.TwipsPerPixelX) + ((Screen.Width / Screen.TwipsPerPixelX) - (frmMain.Width / Screen.TwipsPerPixelX))
    ...
    measure = (frmMain.Height / Screen.TwipsPerPixelY) + ((Screen.Height / Screen.TwipsPerPixelY) - (frmMain.Height / Screen.TwipsPerPixelY))
