...
'            On Error GoTo Render
            ...
            BeginWorld UserControl, Camera.Player


            RenderPlanets UserControl, Camera.Player
            

            'Rotation MakePoint((PI / 2), 0, -(PI / 2)), Molecules("Box1")
            'Rotation MakePoint(frmMain.Evaluate("gpointx") * RADIAN, frmMain.Evaluate("gpointy") * RADIAN, frmMain.Evaluate("gpointz") * RADIAN), Molecules("Box1")

            
            'Rotation MakePoint(frmMain.Evaluate("gpointx") * RADIAN, frmMain.Evaluate("gpointy") * RADIAN, frmMain.Evaluate("gpointz") * RADIAN), Planets("Earth")


          '  Position MakePoint(0.001, 0, 0), Molecules("Box1")
          '  Orientate MakePoint(0, 0.001, 0), Molecules("Box1")
           
            
                       
            mod3DObj.Finish UserControl, Camera.Player

            FinalWorld UserControl, Camera.Player
...
Public Sub PresentScene(ByRef UserControl As Macroscopic)
    ...
    On Error GoTo 0
End Sub
Public Sub BeginWorld(ByRef UserControl As Macroscopic, ByRef MoleculeView As Molecule)
...
'
'        D3DXMatrixRotationX matPitch, MoleculeView.Rotate.x + Camera.Planet.Rotate.x
'        D3DXMatrixMultiply matView, matPitch, matView
'
'        D3DXMatrixRotationY matYaw, -MoleculeView.Rotate.y + Camera.Planet.Rotate.y
'        D3DXMatrixMultiply matView, matYaw, matView
'
'        D3DXMatrixRotationZ matRoll, -MoleculeView.Rotate.z + Camera.Planet.Rotate.z
...
'    Else
    ...
    If Not Camera.Player Is Nothing Then
        If Not Camera.Planet Is Nothing Then
            D3DXMatrixRotationX matPitch, -MoleculeView.Rotate.X - Camera.Planet.Rotate.X '+ (Sin(D720 - MoleculeView.Rotate.x) * (FOVY / (PI * 2.5)))
            D3DXMatrixMultiply matView, matPitch, matView
        
            D3DXMatrixRotationY matYaw, -MoleculeView.Rotate.Y - Camera.Planet.Rotate.Y
            D3DXMatrixMultiply matView, matYaw, matView
            
            D3DXMatrixRotationZ matRoll, -MoleculeView.Rotate.z - Camera.Planet.Rotate.z ' + (Cos(D720 - MoleculeView.Rotate.x) * (FOVY / (PI * 2.5)))
            D3DXMatrixMultiply matView, matRoll, matView
            
            'Set MoleculeView.Origin = VectorRotateAxis(MoleculeView.Origin, Camera.Planet.Rotate)
            Dim loc As Point
            Set loc = VectorRotateAxis(MoleculeView.Origin, Camera.Planet.Rotate)
            
            
            D3DXMatrixTranslation matPos, -loc.X, -loc.Y, -loc.z
            D3DXMatrixMultiply matView, matPos, matView
            
            'Set MoleculeView.Origin = VectorRotateAxis(MoleculeView.Origin, VectorNegative(Camera.Planet.Rotate))
        Else
            D3DXMatrixRotationX matPitch, -MoleculeView.Rotate.X '+ (Sin(D720 - MoleculeView.Rotate.x) * (FOVY / (PI * 2.5)))
            D3DXMatrixMultiply matView, matPitch, matView
        
            D3DXMatrixRotationY matYaw, -MoleculeView.Rotate.Y
            D3DXMatrixMultiply matView, matYaw, matView
            
            D3DXMatrixRotationZ matRoll, -MoleculeView.Rotate.z ' + (Cos(D720 - MoleculeView.Rotate.x) * (FOVY / (PI * 2.5)))
            D3DXMatrixMultiply matView, matRoll, matView
...
'
'        Dim n As Point
'        Set n = PlaneNormal(Camera.Planet.Volume(3).Point3, Camera.Planet.Volume(3).Point2, Camera.Planet.Volume(3).Point1)
'
'        D3DXMatrixLookAtRH matView, ToVector(VectorRotateAxis(MakePoint(0, 0, 1000), MakePoint(0, MoleculeView.Rotate.y, 0))), ToVector(MoleculeView.Origin), ToVector(VectorAddition(MoleculeView.Origin, VectorRotateAxis(MakePoint(0, 1, 0), MakePoint(MoleculeView.Rotate.x, 0, MoleculeView.Rotate.z))))
'    End If
...
Public Sub FinalWorld(ByRef UserControl As Macroscopic, ByRef MoleculeView As Molecule)
...
'
'
'    'If Camera.Planet Is Nothing Then
'
'
