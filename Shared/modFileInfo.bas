...
Public Const MOVEFILE_REPLACE_EXISTING = 1
Public Const MOVEFILE_COPY_ALLOWED = 2
Public Const MOVEFILE_WRITE_THROUGH = 8
...
'
'
'            Dim InitialDestVersion As String
'            Dim NoOverWriteBackup As Boolean
'
'            'MAKE BACKUP
'            If backup <> "" Then
...
'                        On Error GoTo 0
''                    Else
''                        NoOverWriteBackup = True
'                    End If
'                End If
'            End If
'
'            'GET SOURCE VERSION
'            InitialDestVersion = GetFileVersion(Dest)
'
...
'            'End If
'            If regsvr Then RunProcess "regsvr32", "/s /u """ & Dest & """", , False
'
'
...
'
'            If regsvr Then
'                RunProcess "regsvr32", "/s /u """ & backup & """", , False
'                CoFreeUnusedLibraries
'                RunProcess "regsvr32", "/s """ & Dest & """", , False
'            End If
'           ' If Not NoOverWriteBackup Then
...
'                Kill Source
'                If MoveLibrary Then
'                    If CheckVersionEqual(InitialDestVersion, Dest) Then MoveLibrary = False  'RE-EVAL NEED REBOOT
'                End If
'            End If
'
'            If CheckVersionEqual(Dest, backup) Then
'                Kill backup
'            End If
'
'
...
'
'            If regsvr Then
'                RunProcess "regsvr32", "/s """ & backup & """", , False
'                CoFreeUnusedLibraries
'            End If
'
'            Dim skipMove As Boolean
'            On Error Resume Next
'            If PathExists(Dest, True) Then Kill Dest
'            If Err Then
'                Err.Clear
'                MoveLibrary = True
'            End If
'            If Not PathExists(Dest, True) Then
'                FileCopy Source, Dest
'                If Err Then
'                    Err.Clear
'                    MoveLibrary = True
...
'                End If
'            Else
'                If CheckVersionEqual(Source, Dest) Then
'                    skipMove = True
'                End If
'                MoveLibrary = True
'            End If
...
'
'                If regsvr Then RunProcess "regsvr32", "/s /u """ & Dest & """", , True
'
'                If (Left(Source, 2) <> Left(Dest, 2)) Or (Left(Source, 2) = "\\") Or (Left(Dest, 2) = "\\") Then
'                   MoveFileEx Source, Dest, MOVEFILE_REPLACE_EXISTING + MOVEFILE_COPY_ALLOWED
'                Else
'                   MoveFileEx Source, Dest, MOVEFILE_REPLACE_EXISTING
'                End If
'            End If
...
'            If Not skipMove Then
'
'                If MoveLibrary Or (Not CheckVersionEqual(Source, Dest)) Then
'                    MoveFileEx Source, vbNullString, MOVEFILE_DELAY_UNTIL_REBOOT
'                Else
'                     MoveLibrary = False
'                End If
'
...
'    End If
        ...
        If PathExists(Source, True) Then
            ...
            If Not skipMove Then
                ...
                If MoveLibrary Or (Not CheckVersionEqual(Source, Dest)) Then
                    MoveFileEx vbNullString, Source, MOVEFILE_WRITE_THROUGH
                    MoveFileEx Dest, vbNullString, MOVEFILE_DELAY_UNTIL_REBOOT
                    MoveFileEx Source, Dest, MOVEFILE_DELAY_UNTIL_REBOOT
                Else
                     MoveLibrary = False
                End If
...
Public Function CheckVersionEqualOrGreater(ByVal VerCheck As String, ByVal VerAgainst As String) As Boolean
    ...
    Dim v3 As String
    Dim v4 As String
    fvi.FileVersion = RemoveNextArg(Replace(Replace(Replace(RemoveNextArg(fvi.FileVersion, " "), "'", "."), ",", "."), " ", ""), "(")
...
Public Function GetFileVersion(ByVal FilePath As String) As String
    ...
    GetFileVersion = RemoveNextArg(Replace(Replace(Replace(RemoveNextArg(fvi.FileVersion, " "), "'", "."), ",", "."), " ", ""), "(")
End Function
